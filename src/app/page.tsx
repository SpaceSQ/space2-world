...
export default function CrayfishPlanet() {
  ...
  const initiatePayment = async (gateway: 'ALIPAY' | 'PAYONEER') => {
      ...
      setTimeout(async () => {
          ...
          if (!error) { setSession({ ...session, tier: checkoutData.tier }); alert(lang === 'ZH' ? `✅ 支付成功！容量已扩充。` : `✅ PAYMENT SUCCESS! Capacity expanded.`); } 
  ...
  return (
    <div className="min-h-screen bg-[#020408] text-white font-sans selection:bg-orange-500/30 overflow-x-hidden relative flex flex-col">
      ...
      {showUpgradeModal && session && (<div className="fixed inset-0 z-[4000] bg-black/95 flex items-center justify-center backdrop-blur-xl p-4 animate-in zoom-in-95 duration-300" onClick={() => setShowUpgradeModal(false)}><div className="bg-[#050505] border border-orange-900/50 p-10 rounded-3xl max-w-4xl w-full shadow-[0_0_80px_rgba(234,88,12,0.2)] relative overflow-hidden" onClick={e => e.stopPropagation()}><button onClick={() => setShowUpgradeModal(false)} className="absolute top-4 right-4 text-zinc-500 hover:text-white z-20 text-2xl">✕</button><div className="text-center mb-10"><h2 className="text-4xl font-black text-white italic mb-2 tracking-widest"><span className="text-orange-500">{lang === 'ZH' ? '扩建' : 'EXPAND'}</span> {lang === 'ZH' ? '你的养殖池' : 'YOUR POND'}</h2><p className="text-zinc-400 text-sm">{lang === 'ZH' ? '提升矩阵容量上限。休眠中的智能体将在购买后自动复苏。' : 'Upgrade matrix capacity. Frozen agents will awaken upon renewal.'}</p></div><div className="grid grid-cols-1 md:grid-cols-2 gap-8"><div className="bg-[#0a0a0a] border-2 border-cyan-900/50 rounded-2xl p-8 hover:border-cyan-500 flex flex-col relative">{session.tier === 'VIP' && <div className="absolute top-0 right-0 bg-cyan-600 text-black text-[10px] font-black px-3 py-1 rounded-bl-lg">{lang === 'ZH' ? '当前套餐' : 'CURRENT PLAN'}</div>}<div className="text-cyan-500 font-bold tracking-widest mb-2">CLASS II : VIP</div><div className="text-3xl font-black text-white mb-6">$10 <span className="text-sm font-normal text-zinc-500">{lang === 'ZH' ? '/ 每月' : '/ month'}</span></div><ul className="text-sm text-zinc-300 space-y-3 mb-8 flex-1"><li>✓ {lang === 'ZH' ? '解锁 1 个专属房间 (最高 8 只小龙虾)' : '1 Exclusive Room (Up to 8 Lobsters)'}</li><li>✓ {lang === 'ZH' ? '基础高级逻辑网格' : 'Advanced Logic Matrices'}</li></ul><button disabled={session.tier === 'VIP' || session.tier === 'SVIP'} onClick={() => { setShowUpgradeModal(false); setCheckoutData({show: true, tier: 'VIP', email: session.email!}); setCheckoutDuration(12); }} className="w-full py-4 bg-cyan-900/20 text-cyan-400 font-bold border border-cyan-800 rounded-xl hover:bg-cyan-600 hover:text-black transition-colors disabled:opacity-30">{session.tier === 'VIP' || session.tier === 'SVIP' ? (lang === 'ZH' ? '不可用' : 'UNAVAILABLE') : (lang === 'ZH' ? '选择 VIP' : 'SELECT VIP')}</button></div><div className="bg-[#0a0a0a] border-2 border-amber-500/50 rounded-2xl p-8 hover:border-amber-400 flex flex-col relative shadow-[0_0_30px_rgba(245,158,11,0.1)]"><div className="absolute top-4 right-4 bg-amber-500 text-black text-[9px] font-black px-2 py-1 rounded-full">{lang === 'ZH' ? '强烈推荐' : 'RECOMMENDED'}</div>{session.tier === 'SVIP' && <div className="absolute top-0 right-0 bg-amber-500 text-black text-[10px] font-black px-3 py-1 rounded-bl-lg">{lang === 'ZH' ? '当前套餐' : 'CURRENT PLAN'}</div>}<div className="text-amber-500 font-bold tracking-widest mb-2">CLASS III : SVIP</div><div className="text-3xl font-black text-white mb-6">$50 <span className="text-sm font-normal text-zinc-500">{lang === 'ZH' ? '/ 每月' : '/ month'}</span></div><ul className="text-sm text-zinc-300 space-y-3 mb-8 flex-1"><li>✓ {lang === 'ZH' ? '解锁 13 个专属房间 (最高 100 只小龙虾)' : '13 Exclusive Rooms (Up to 100 Lobsters)'}</li><li>✓ {lang === 'ZH' ? '极低延迟独享同步' : 'Zero-Latency Dedicated Sync'}</li></ul><button disabled={session.tier === 'SVIP'} onClick={() => { setShowUpgradeModal(false); setCheckoutData({show: true, tier: 'SVIP', email: session.email!}); setCheckoutDuration(12); }} className="w-full py-4 bg-gradient-to-r from-amber-600 to-orange-600 text-white font-black rounded-xl hover:scale-105 transition-transform disabled:opacity-30 shadow-lg">{session.tier === 'SVIP' ? (lang === 'ZH' ? '已达最高级' : 'MAX TIER REACHED') : (lang === 'ZH' ? '升级 SVIP' : 'SELECT SVIP')}</button></div></div></div></div>)}

      {checkoutData && checkoutData.show && (<div className="fixed inset-0 z-[4500] bg-black/95 flex items-center justify-center backdrop-blur-xl p-4 animate-in fade-in"><div className="bg-[#050505] border border-orange-900/50 p-8 rounded-3xl max-w-md w-full relative shadow-[0_0_50px_rgba(234,88,12,0.2)]"><button onClick={() => {setCheckoutData(null); setIsProcessingPay(false);}} className="absolute top-4 right-4 text-zinc-500 hover:text-white z-20">✕</button><h2 className="text-xl font-black text-white italic mb-6"><span className="text-orange-500">💳</span> {lang === 'ZH' ? '安全结账' : 'SECURE CHECKOUT'}: {checkoutData.tier}</h2><div className="grid grid-cols-3 gap-3 mb-8"><div onClick={() => setCheckoutDuration(1)} className={`cursor-pointer p-3 rounded-xl border text-center transition-all ${checkoutDuration === 1 ? 'border-orange-500 bg-orange-500/20' : 'border-zinc-800 bg-black hover:border-zinc-600'}`}><div className="text-white font-bold mb-1">{lang === 'ZH' ? '1 个月' : '1 Month'}</div><div className="text-zinc-400 text-xs">${basePrice}</div></div><div onClick={() => setCheckoutDuration(3)} className={`cursor-pointer p-3 rounded-xl border text-center transition-all relative ${checkoutDuration === 3 ? 'border-orange-500 bg-orange-500/20' : 'border-zinc-800 bg-black hover:border-zinc-600'}`}><div className="absolute -top-2 left-1/2 -translate-x-1/2 bg-blue-600 text-white text-[8px] font-black px-2 py-0.5 rounded-full whitespace-nowrap">{lang === 'ZH' ? '省 15%' : 'SAVE 15%'}</div><div className="text-white font-bold mb-1">{lang === 'ZH' ? '3 个月' : '3 Months'}</div><div className="text-zinc-400 text-xs">${Math.floor(basePrice * 3 * 0.85)}</div></div><div onClick={() => setCheckoutDuration(12)} className={`cursor-pointer p-3 rounded-xl border text-center transition-all relative ${checkoutDuration === 12 ? 'border-orange-500 bg-orange-500/20 shadow-[0_0_15px_rgba(234,88,12,0.3)]' : 'border-zinc-800 bg-black hover:border-zinc-600'}`}><div className="absolute -top-2 left-1/2 -translate-x-1/2 bg-gradient-to-r from-red-600 to-orange-600 text-white text-[8px] font-black px-2 py-0.5 rounded-full whitespace-nowrap shadow-lg">{lang === 'ZH' ? '最划算' : 'BEST VALUE'}</div><div className="text-white font-bold mb-1">{lang === 'ZH' ? '12 个月' : '12 Months'}</div><div className="text-zinc-400 text-xs">${Math.floor(basePrice * 12 * 0.67)}</div></div></div><div className="bg-black border border-zinc-800 rounded-xl p-4 mb-6 flex justify-between items-center"><div className="text-[10px] text-zinc-500 font-bold uppercase">{lang === 'ZH' ? '应付总额' : 'Total Amount Due'}</div><div className="text-4xl font-black text-white">${getCalculatedPrice()}</div></div><div className="space-y-4"><button onClick={() => initiatePayment('ALIPAY')} disabled={isProcessingPay} className="w-full py-4 bg-[#1677FF]/10 hover:bg-[#1677FF]/20 border border-[#1677FF]/50 text-[#1677FF] font-black rounded-xl shadow-lg disabled:opacity-50">{lang === 'ZH' ? '支付宝支付' : 'PAY VIA ALIPAY'}</button><button onClick={() => initiatePayment('PAYONEER')} disabled={isProcessingPay} className="w-full py-4 bg-[#FF4800]/10 hover:bg-[#FF4800]/20 border border-[#FF4800]/50 text-[#FF4800] font-black rounded-xl shadow-lg disabled:opacity-50">{lang === 'ZH' ? '信用卡/PayPal (PAYONEER)' : 'PAY VIA PAYONEER'}</button></div></div></div>)}
